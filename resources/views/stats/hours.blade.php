@section('site_title', formatTitle([$link->alias, __('Hours'), __('Stats'), config('settings.title')]))
        ...
        <div class="row">
            ...
            <div class="col-auto">
                <div class="btn-group btn-group-sm" role="group" aria-label="Basic example">
                    <a href="{{ route('stats.evolution.hours', ['id' => $link->id]) }}" class="btn btn-primary">{{ __('Hours') }}</a>
                            ...
                            <div class="d-flex justify-content-between mb-2">
                                <div class="d-flex text-truncate align-items-center">
                                    <div class="text-truncate">
                                        {{ \Carbon\Carbon::parse($value['date_result'])->tz(Auth::user()->timezone ?? config('app.timezone'))->format('g A') }} <span class="text-muted">{{ __(\Carbon\Carbon::parse($value['date_result'])->tz(Auth::user()->timezone ?? config('app.timezone'))->format('l')) }}</span>
                                    </div>
                                </div>
